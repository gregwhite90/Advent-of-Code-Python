...
from typing import Union, NamedTuple, List, Tuple, DefaultDict
...
class Solution(SolutionABC):
  ...
  def period(self, idx: int):
    steps = 0
    history: DefaultDict[Tuple[int], List[int]] = defaultdict(list)
    while True:
      status = tuple(([moon.pos[idx]for moon in self.moons] + [moon.vel[idx] for moon in self.moons]))
      history[status].append(steps)
      if len(history[status]) > 1:
        break
      else:
        self.time_step()
        steps += 1
  ...
  def solve(
    ...
    return (self.period(0), self.period(1), self.period(2))